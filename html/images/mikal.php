		...
		foreach($cursor as $document){
			for($matchNumber = 1; $matchNumber <= CountMatchesScheduleInputTimesFour($dataValidation)/4; $matchNumber++){
				foreach($redRedCombination as $currentColorRed){
					...
					foreach ($matchesPlayedByThatTeamAndAlliance['Red'][count($matchesPlayedByThatTeamAndAlliance['Red'])] as $key ) {
						echo 'KEY: ' . $key . '<br />';
					}
					for ($i=0; $i <= 100; $i++) { 
						echo 'dang: '. $matchesPlayedByThatTeamAndAlliance['Red'][$i] . '<br />';
					}
					/*
